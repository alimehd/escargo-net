...
function saveAuth(data) {
  localStorage.setItem('escargo_auth', JSON.stringify(data))
}

function clearAuth() {
  localStorage.removeItem('escargo_auth')
}
...
function formatDisplayDate(str) {
  if (!str) return ''
  const d = parseLocalDate(str.substring(0, 10))
...
export default function ReservationsPage() {
  ...
  const [flow,         setFlow]         = useState('email') // email | pin | setup | unknown | ready
  const [emailInput,   setEmailInput]   = useState('')
  const [pinInput,     setPinInput]     = useState('')
  const [nameInput,    setNameInput]    = useState('')
  const [pinConfirm,   setPinConfirm]   = useState('')
  const [userMeta,     setUserMeta]     = useState(null)  // {email, userType, setupComplete}
  ...
  // Active tab
  const [tab, setTab] = useState('calendar') // calendar | mine | admin

  // Booking form
  const [showForm,   setShowForm]   = useState(false)
  const [editId,     setEditId]     = useState(null)
  const [formData,   setFormData]   = useState({
    spotType: 'bed', spotNumber: 1,
    startDate: '', endDate: '',
  ...
  // Access request (unknown visitors)
  ...
  async function handlePinSubmit(e) {
    ...
    try {
      const data = await apiFetch('/auth/verify', {
        method: 'POST',
        body: JSON.stringify({ email: userMeta.email, pin: pinInput }),
      })
      const authData = { email: userMeta.email, pin: pinInput, ...data.user }
      saveAuth(authData)
      setAuth(authData)
      setFlow('ready')
      loadReservations(authData)
    } catch (e) {
      setError(e.message)
      setPinInput('')
    } finally {
  ...
  async function handleAccessRequest(e) {
    ...
    try {
      await apiFetch('/access-request', {
        method: 'POST',
        body: JSON.stringify({ email: userMeta.email, name: reqName.trim() }),
      })
  ...
  function openNewBooking(spotType = 'bed', spotNumber = 1, date = '') {
    setFormData({ spotType, spotNumber, startDate: date, endDate: date, notes: '', isPublic: false })
    setEditId(null)
    setFormError('')
    setShowForm(true)
  }
  ...
  async function handleFormSubmit(e) {
    ...
    try {
      if (editId) {
        await apiFetch(`/reservations/${editId}`, {
          method: 'PUT',
  ...
  async function handleRemoveUser(email) {
    if (!window.confirm(`Remove ${email} from permitted users?`)) return
    try {
      await apiFetch('/admin/users', {
        method: 'DELETE',
  ...
  function getSpotReservations(spotType, spotNumber, year, month) {
    const start = new Date(year, month, 1)
    const end   = new Date(year, month + 1, 0)
    return reservations.filter(r =>
      r.spot_type === spotType &&
      r.spot_number === spotNumber &&
      parseLocalDate(r.end_date.substring(0, 10)) >= start &&
      parseLocalDate(r.start_date.substring(0, 10)) <= end
    )
  }
  ...
  return (
    <div className="rsp">
      <div className="rsp__topbar">
        <a href="/" className="rsp__back">L'Es-Cargo</a>
          ...
          <div className="rsp__card rsp__card--narrow">
            ...
            <form onSubmit={handlePinSubmit} className="rs-form">
              <label className="rs-label">
                4-digit PIN
                <input
                  className="rs-input rs-input--pin"
                  type="password"
                  inputMode="numeric"
                  maxLength={4}
                  value={pinInput}
            ...
            {!reqSubmitted ? (
              <>
                ...
                <p className="rsp__card-sub">
                  This email does not have access to the reservation system yet.
                  Submit a request and you should receive a response within a few days.
                </p>
                <form onSubmit={handleAccessRequest} className="rs-form">
                  ...
                  <button
                    type="button"
                    className="rs-btn rs-btn--ghost"
                    onClick={() => { setFlow('email'); setError('') }}
                  >
                    Back
                  </button>
                </form>
              ...
              <>
                ...
                <p className="rsp__card-sub">
                  Your request has been received. The hosts will review it and you should hear back
                  within a few days.
                </p>
                <button
                  className="rs-btn rs-btn--ghost"
        ...
        {flow === 'ready' && (
          <div className="rsp__main">
            ...
            <div className="rs-tabs">
              <button
                className={`rs-tab ${tab === 'calendar' ? 'rs-tab--active' : ''}`}
                onClick={() => setTab('calendar')}
              >
                Calendar
              </button>
              <button
                className={`rs-tab ${tab === 'mine' ? 'rs-tab--active' : ''}`}
                onClick={() => setTab('mine')}
              >
                My Reservations
              </button>
              {isAdmin && (
                <button
                  className={`rs-tab ${tab === 'admin' ? 'rs-tab--active' : ''}`}
              ...
              <div className="rsp__section">
                <div className="rscal">
                  <div className="rscal__nav">
                    ...
                    <button
                      className="rs-btn rs-btn--secondary rscal__new-btn"
                  ...
                  <div className="rscal__legend">
                    ...
                    {isAdmin && (
                      <span className="rscal__leg rscal__leg--private">Private (admin view)</span>
                    )}
                  ...
                  {resLoading ? (
                    ...
                    <CalendarGrid
                      ...
                      isAdmin={isAdmin}
                      onCellClick={(spotType, spotNumber, date) => {
                        const r = cellReservation(spotType, spotNumber, calYear, calMonth, date)
                        if (r && (r.user_email === auth.email || isAdmin)) {
                          openEditBooking(r)
                        } else if (!r) {
                          openNewBooking(spotType, spotNumber, isoDate(new Date(calYear, calMonth, date)))
                        }
                      }}
...
function CalendarGrid({ year, month, authEmail, isAdmin, onCellClick, cellReservation, cellClass, cellTitle }) {
      ...
      <table className="rscal__table">
        <thead>
          <tr>
            ...
            {dayNums.map(d => (
              <th key={d} className="rscal__th-day">{d}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          ...
          {SPOT_NUMBERS.map(num => (
            <tr key={`bed-${num}`}>
              <td className="rscal__spot-label">Bed {num}</td>
              {dayNums.map(d => {
                const r = cellReservation('bed', num, year, month, d)
                const cls = cellClass(r, authEmail)
                const title = cellTitle(r)
                const clickable = !r || r.user_email === authEmail || isAdmin
                return (
                  <td
                    key={d}
                    className={`${cls} ${clickable ? 'rscal__cell--clickable' : ''}`}
                    title={title}
                    onClick={() => onCellClick('bed', num, d)}
                  />
                )
              })}
            </tr>
          ...
          {SPOT_NUMBERS.map(num => (
            <tr key={`camp-${num}`}>
              <td className="rscal__spot-label">Camp {num}</td>
              {dayNums.map(d => {
                ...
                return (
                  <td
                    ...
                    className={`${cls} ${clickable ? 'rscal__cell--clickable' : ''}`}
                    title={title}
                    onClick={() => onCellClick('camp', num, d)}
                  />
                )
              })}
            </tr>
...
function ReservationList({ reservations, authEmail, isAdmin, onEdit, onDelete }) {
  ...
  if (!sorted.length) {
    return <p className="rsp__empty">No reservations to show.</p>
  }
        ...
        return (
          <li key={r.id} className="rslist__item">
            <div className="rslist__spot">
              {r.spot_type === 'bed' ? 'Bed' : 'Camp Spot'} {r.spot_number}
              {r.is_public
      ...
      <div className="rsmodal">
        ...
        <form onSubmit={onSubmit} className="rs-form">
          ...
          <div className="rsmodal__footer">
            <button type="button" className="rs-btn rs-btn--ghost" onClick={onClose}>
              Cancel
            </button>
...
function AdminPanel({
  users, requests, newUserEmail, setNewUserEmail, adminError,
  ...
  return (
    <div className="rsp__section">
      <div className="rs-tabs rs-tabs--sub">
        <button
          className={`rs-tab rs-tab--sub ${adminTab === 'reservations' ? 'rs-tab--active' : ''}`}
          onClick={() => setAdminTab('reservations')}
        >
          All Reservations
        </button>
        <button
          className={`rs-tab rs-tab--sub ${adminTab === 'users' ? 'rs-tab--active' : ''}`}
          onClick={() => setAdminTab('users')}
        >
          Manage Access
        </button>
        <button
          className={`rs-tab rs-tab--sub ${adminTab === 'requests' ? 'rs-tab--active' : ''}`}
        ...
        <div>
          ...
          <table className="rs-table">
            ...
            <tbody>
              {users.map(u => (
                <tr key={u.id}>
                  ...
                  <td>
                    {u.role !== 'admin' && (
                      <button
                        className="rs-btn rs-btn--small rs-btn--danger"
              ...
              {!users.length && (
                <tr><td colSpan={5} className="rs-muted">No users yet.</td></tr>
              )}
            </tbody>
          </table>
        </div>
            ...
            <tbody>
              {requests.map(r => (
                <tr key={r.id}>
                  ...
                  <td>
                    <span className={`rs-status rs-status--${r.status}`}>{r.status}</span>
                  </td>
                  <td>
                    {r.status === 'pending' && (
                      <div className="rslist__actions">
                        <button
                          className="rs-btn rs-btn--small rs-btn--approve"
                        ...
                        <button
                          className="rs-btn rs-btn--small rs-btn--danger"
                          onClick={() => onUpdateRequest(r.id, 'denied')}
                        >
                          Deny
                        </button>
                      </div>
              ...
              {!requests.length && (
                <tr><td colSpan={5} className="rs-muted">No requests yet.</td></tr>
              )}
            </tbody>
          </table>
        </div>
